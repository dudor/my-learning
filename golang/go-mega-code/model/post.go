...
func GetPostsByUserIDPageAndLimit(id,page,limit int) (*[]Post,int,error){
	...
	var posts []Post
	offset := (page-1)*limit
	if err:= db.Debug().Preload("User").Order("timestamp desc").Where("user_id=?",id).Offset(offset).Limit(limit).Find(&posts).Error;err!=nil{
...
func GetPostsByPageAndLimit(page,limit int)(*[]Post,int ,error)  {
	...
	var posts []Post
	offset := (page-1)*limit
	if err:= db.Debug().Preload("User").Order("timestamp").Offset(offset).Limit(limit).Find(&posts).Error;err!=nil{
		return nil,total,err
	}
